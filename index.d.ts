// Type definitions for TSTL v1.4.1
...
declare namespace std.base {
    ...
    abstract class _MapTree<Key, T> extends _XTree<MapIterator<Key, T>> {
        ...
        abstract find_by_key(key: Key): _XTreeNode<MapIterator<Key, T>>;
        lower_bound(key: Key): MapIterator<Key, T>;
...
declare namespace std.base {
    class _MultiMapTree<Key, T> extends _MapTree<Key, T> {
        constructor(map: TreeMultiMap<Key, T>, compare: (x: Key, y: Key) => boolean);
        insert(val: MapIterator<Key, T>): void;
        find_by_key(key: Key): _XTreeNode<MapIterator<Key, T>>;
        upper_bound(key: Key): MapIterator<Key, T>;
    }
}
declare namespace std.base {
    ...
    abstract class _SetTree<T> extends _XTree<SetIterator<T>> {
         ...
         * Default Constructor.
         */
        constructor(set: ITreeSet<T>, compare: (x: T, y: T) => boolean, itCompare: (x: SetIterator<T>, y: SetIterator<T>) => boolean);
        abstract find_by_val(val: T): _XTreeNode<SetIterator<T>>;
        lower_bound(val: T): SetIterator<T>;
...
declare namespace std.base {
     ...
     * @hidden
     */
    class _UniqueMapTree<Key, T> extends _MapTree<Key, T> {
        constructor(map: TreeMap<Key, T>, compare: (x: Key, y: Key) => boolean);
        find_by_key(key: Key): _XTreeNode<MapIterator<Key, T>>;
        upper_bound(key: Key): MapIterator<Key, T>;
    }
}
declare namespace std.base {
     ...
     * @hidden
     */
    class _UniqueSetTree<T> extends _SetTree<T> {
        constructor(set: TreeSet<T>, compare: (x: T, y: T) => boolean);
        find_by_val(val: T): _XTreeNode<SetIterator<T>>;
        upper_bound(val: T): SetIterator<T>;
    }
}
